...
export default function SocialProofGrid() {
  ...
  const { scrollYProgress } = useScroll({
    target: sectionRef,
    offset: ["0 1", "0.6 1"] // Empieza a animar cuando asoma, termina cuando el 60% ha entrado
  });

  const gridY = useTransform(scrollYProgress, [0, 1], [100, 0]);
  const gridOpacity = useTransform(scrollYProgress, [0, 1], [0, 1]);

  return (
    <section ref={sectionRef} className="py-20 md:py-32 bg-base border-t border-ink/5">
