...
export function CartDrawer() {
  ...
  return (
    <>
      {isCartOpen && (
        <div 
          className="fixed inset-0 bg-ink/30 backdrop-blur-md z-40 transition-opacity duration-700 ease-[cubic-bezier(0.25,1,0.5,1)]"
          onClick={() => setCartOpen(false)}
        />
      )}
      
      <aside 
        className={`fixed top-0 right-0 z-50 w-full max-w-md h-full bg-base flex flex-col shadow-[-20px_0_40px_-10px_rgba(44,42,41,0.1)] transform transition-transform duration-700 ease-[cubic-bezier(0.25,1,0.5,1)] border-l border-ink/5 ${
          isCartOpen ? 'translate-x-0' : 'translate-x-full'
        }`}
      >
        <header className="flex items-center justify-between px-6 py-8 border-b border-ink/5">
          <h2 className="font-serif text-2xl text-ink font-medium tracking-wide">Tu paquete de Sevilla</h2>
          <button 
            aria-label="Cerrar carrito" 
            className="p-2 text-ink/60 hover:text-ink transition-colors duration-300 rounded-full hover:bg-ink/5 focus-visible:ring-1 focus-visible:ring-offset-2 focus-visible:outline-none focus-visible:ring-ink/20 active:scale-[0.94]"
            onClick={() => setCartOpen(false)}
          >
